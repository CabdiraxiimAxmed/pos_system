    ...
    <>
      ...
      {/* {editCustomer && <EditCustomer />} */}
        <Stack direction="row" alignItems="center" justifyContent="space-between" mb={5} style={{marginBottom : 9}}>
          ...
          <Button variant="contained" onClick={() => setAddProduct(!addProduct) } startIcon={<AddIcon />} style={{backgroundColor:"#2367d1", fontWeight: 'bold'}}>
            badeeco
          </Button>
        </Stack>
        ...
        <div className="filters-container">
          <div className="dropdown">
            ...
            <div className="dropdown-content">
              {Object.keys(products[0]).map((column_head: string, index: number) => (
      ...
      <div className="table-container">
        <table>
          ...
          {products.map((product: ProductType, index: number) => (
            <tbody key={index}>
              <tr>
                <td className={display('id')? '': 'inactive'}>{product.id}</td>
                <td className="user-name">{product.name}</td>
                <td className={display('supplier')? '': 'inactive'}>{product.supplier}</td>
                <td className={display('quantity')? '': 'inactive'}>{product.quantity}</td>
                <td className={display('alertquantity')? '': 'inactive'}>{product.alertquantity}</td>
                <td className={display('barcode')? '': 'inactive'}>{product.barcode}</td>
                <td className={display('price')? '': 'inactive'}>{product.price}</td>
              </tr>
            </tbody>
          ))}
        </table>
