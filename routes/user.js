...
router.get('/:username', async(req, res) => {
  ...
  try {
    let resp = await client.query(`SELECT * FROM user_info WHERE username = '${username}'`);
...
router.post('/create-user', async(req, res) => {
  ...
  try {
    let resp = await client.query(`INSERT INTO user_info (name, username, role, contact, permissions, password, created_date) VALUES('${name}', '${username}', '${role}', '${contact}', '{"${permissions}"}', '${hash}', DEFAULT)`);
    res.send('success');
  } catch(err) {
    res.send('error');
  }
...
router.post('/update-user', async(req, res) => {
  ...
  let hash = CryptoJS.AES.encrypt(password, 'ahmed').toString();
  try {
    const resp = await client.query(`UPDATE user_info SET name='${name}', username='${username}', role='${role}', contact='${contact}', password='${hash}', permissions=array['${permissions}']::varchar[] WHERE id='${id}'`);
    res.send('success');
  } catch(err) {
    console.log(err);
    res.send('error');
...
router.post('/delete/:username', async(req, res) => {
  const { username } = req.params;
  try {
    const resp = await client.query(`DELETE FROM user_info WHERE username='${username}'`);
    res.send('success');
  } catch(err) {
    res.send("error");
  }
